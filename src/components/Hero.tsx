...
import type { StatPillConfig } from "@/config/verticals/types";
        ...
        <div className="flex-1 w-full lg:max-w-xl">
          ...
          >
            <span className="block" style={{ marginBottom: "0.05em" }}>
              {hero.headline.line1Words.map((word, i) => (
                <span key={i}>
                  <WordSpan word={word} delay={i} />
                  {i < hero.headline.line1Words.length - 1 && " "}
                </span>
              ))}
            </span>
            <span className="block" style={{ marginBottom: "0.05em" }}>
              {hero.headline.line2Words.map((word, i) => (
                <span key={i}>
                  <WordSpan word={word} delay={hero.headline.line1Words.length + i} />
                  {i < hero.headline.line2Words.length - 1 && " "}
                </span>
              ))}
            </span>
          ...
          >
            ...
            <span style={{
              ...
              backgroundClip: "text",
              fontWeight: 500,
            }}>{hero.subheadline.teamHighlight}</span>{" "}
            {hero.subheadline.after}{" "}
            <span className="text-white font-medium">
              {hero.subheadline.callout}
            </span>
          </p>
          ...
          <div className={`flex flex-col sm:flex-row flex-wrap gap-2.5 sm:gap-3 ${cls("cta")}`}>
            {hero.stats.map((stat: StatPillConfig, i: number) => (
              <StatPill
                key={i}
                value={stat.value}
                suffix={stat.suffix}
